...
type Day04Fixture struct {
	*gunit.Fixture
}

func TestDay04Fixture(t *testing.T) {
	gunit.Run(new(Day04Fixture), t)
}

func (this *Day04Fixture) Setup() {
}

func (this *Day04Fixture) Test_OneStringParsesCorrectly() {
	roomCheck := NewRoomCheck([]string{})
	result := roomCheck.ParseRoom("aaaaa-bbb-z-y-x-123[abxyz]")
	this.So(result, should.Resemble, []interface{}{"aaaaabbbzyx", 123, "abxyz"})
}

func (this *Day04Fixture) Test_TwoStringsParseCorrectly() {
	roomCheck := NewRoomCheck([]string{"aaaaa-bbb-z-y-x-123[abxyz]","a-b-c-d-e-f-g-h-987[abcde]"})
	sectorSum := roomCheck.CheckAllRooms()
	this.So(sectorSum, should.Resemble, 1110)
}

func (this *Day04Fixture) Test_ExampleRoomsWork() {
	roomCheck := NewRoomCheck([]string{"aaaaa-bbb-z-y-x-123[abxyz]","a-b-c-d-e-f-g-h-987[abcde]","not-a-real-room-404[oarel]","totally-real-room-200[decoy]"})
	sectorSum := roomCheck.CheckAllRooms()
	this.So(sectorSum, should.Resemble, 1514)
}

func (this *Day04Fixture) Test_CountRoomNameCharacters() {
	charMap := countChars("unggabung")
	this.So(charMap, should.Resemble, map[rune]int{103:3, 97:1, 98:1, 117:2, 110:2})
}

func (this *Day04Fixture) Test_SortRoomNameCharacters() {
	testMap := map[rune]int{97:2, 98:1, 101:1, 108:2, 111:1, 114:1, 115:1, 117:1, 121:1}
	sorted := sortRoomNameCharacters(testMap)
	this.So(sorted, should.Resemble, []rune{97, 108, 98, 101, 111})
}

func (this *Day04Fixture) Test_HighestNumberFound() {
	testMap := map[rune]int{97:2, 98:1, 101:1, 108:2, 111:1, 114:1, 115:1, 117:7, 121:1}
	highestNumber := getHighestNumber(testMap)
	this.So(highestNumber, should.Equal, 7)
}

func (this *Day04Fixture) Test_ThePuzzleInput() {
	roomCheck := NewRoomCheck(thePuzzleInput)
	sectorSum := roomCheck.CheckAllRooms()
	this.So(sectorSum, should.Resemble, 137896)
}

/*
Design:
- parse letters of room name, count letters, score letters
- compare letters to checksum
- if "room" is "real" add sector to "real rooms" total
 */

var thePuzzleInput = []string{
