...
func main() {
	for _, id := range input {
		for _, comparingID := range input {
			if bothIDsAreOneLetterOff(id, comparingID) {
				fmt.Println("id:", id)
				fmt.Println("comparingID:", comparingID)
			}
		}
	}
}

func bothIDsAreOneLetterOff(idOne, idTwo string) bool {
	differenceCount := 0
	for index, letter := range idOne {
		if int32(idTwo[index]) != letter {
			differenceCount++
		}
		//fmt.Println("idTwo[index]:", idTwo[index], ", letter:", letter)
	}
	//fmt.Println("differenceCount:", differenceCount)
