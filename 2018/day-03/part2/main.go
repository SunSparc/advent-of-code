...
func main() {
	file, err := os.Open("../input.test") // input.test
	if err != nil {
...
func splitClaimIntoComponents(claim string) (int, Coordinate, Size) {
	// #1 @ 871,327: 16x20
	fourParts := strings.Split(claim, " ")

	claimId, _ := strconv.Atoi(strings.Replace(fourParts[0], "#", "", 1))
	...
	return claimId, topLeftCoordinate, size
}

func recordClaimCoordinates(claimID int, topLeftCoordinate Coordinate, size Size) {
